...
class BuiltInLogin(APIView):
    ...
    def post(self, request):
        ...
        try:
            if not CustomUser.objects.filter(email=email, is_google=0).exists():
                return Response({"data": {}, "error": "Account with this email does not exist"}, status=status.HTTP_400_BAD_REQUEST)
            if not CustomUser.objects.filter(email=email, password=password, is_google=0).exists():
                return Response({"data": {}, "error": "Invald credentials"}, status=status.HTTP_400_BAD_REQUEST)
            user = CustomUser.objects.get(
                email=email, password=password, is_google=0)
        ...
        try:
            ...
            user, created = CustomUser.objects.get_or_create(
                ...
                defaults={"first_name": first_name,
                          "last_name": last_name, "email": email, "is_google": 1}
            )
